...
layui.define('form', function (exports) {
  ...
  Class.prototype.setCheckbox = function (elem, item, elemCheckbox) {
    ...
    if (typeof item.children === 'object' || elem.find('.' + ELEM_PACK)[0]) {
      ...
      childs.each(function () {
        ...
        // update-begin--Author:sunjianlei Date:20191216 for：取消选中父节点的时候同步选中所有子节点
        // this.checked = checked
        // update-end--Author:sunjianlei Date:20191216 for：取消选中父节点的时候同步选中所有子节点
      })
    }
    ...
    var setParentsChecked = function (thisNodeElem) {
      ...
      // update-begin--Author:sunjianlei Date:20191216 for：取消选中子节点的时候同步选中父节点
      //向父节点递归
      // setParentsChecked(parentNodeElem)
    }

    // setParentsChecked(elem)
    // update-end--Author:sunjianlei Date:20191216 for：取消选中子节点的时候同步选中父节点
  ...
  Class.prototype.getChecked = function () {
    ...
    // update-begin--Author:sunjianlei Date:20191216 for：修改因取消父子关联导致的选中节点无法获取的问题
    //遍历节点找到选中索引
    var a=that.elem.find('.layui-form-checked')
    a.each(function () {
      checkId.push($(this).prev()[0].value)
    })

    //遍历节点
    var eachNodes = function (item2, data, checkNode) {
      layui.each(data, function (index, item) {
        if (item.id == item2) {
          ...
          if (item.children) {
            cloneItem.children = []
            // eachNodes(item2, item.children, cloneItem.children)
            eachNodes(item2, item.children, checkNode)
          }
          return true
        } else if (item.children) {
          return eachNodes(item2, item.children, checkNode)
        }
      })

    }
    layui.each(checkId, function (index2, item2) {
      eachNodes(item2, $.extend({}, options.data), checkData)
    })
    // update-end--Author:sunjianlei Date:20191216 for：修改因取消父子关联导致的选中节点无法获取的问题
