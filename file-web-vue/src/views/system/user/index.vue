<template>
  <a-row :gutter="24">
    ...
    <a-col :md="24" :sm="24">
      <x-card v-if="hasPerm('GET /sysUser/list')">
      ...
      <a-card :bordered="false">
        ...
        >
          <template slot="operator">
            <a-button type="primary" v-if="hasPerm('POST /sysUser/add')" icon="plus" @click="$refs.addForm.add()">新增用户</a-button>
            <a-button type="danger" :disabled="selectedRowKeys.length < 1" v-if="hasPerm('POST /sysUser/delete')" @click="batchDelete"><a-icon type="delete"/>批量删除</a-button>
            <x-down
          ...
          <span slot="status" slot-scope="text,record" v-if="hasPerm('POST /sysUser/changeStatus')">
          ...
          <span slot="action" slot-scope="text, record">
            <a v-if="hasPerm('POST /sysUser/edit')" @click="$refs.editForm.edit(record)">编辑</a>
            <a-divider type="vertical" v-if="hasPerm('POST /sysUser/edit')" />
            <a-dropdown v-if="hasPerm('POST /sysUser/resetPwd') || hasPerm('POST /sysUser/grantRole') || hasPerm('POST /sysUser/delete')">
              <a class="ant-dropdown-link">
                更多 <a-icon type="down" />
              </a>
              <a-menu slot="overlay">
                <a-menu-item v-if="hasPerm('POST /sysUser/resetPwd')">
                  <a-popconfirm placement="topRight" title="确认重置密码？" @confirm="() => resetPwd(record)">
                    <a>重置密码</a>
                  </a-popconfirm>
                </a-menu-item>
                <a-menu-item v-if="hasPerm('POST /sysUser/grantRole')">
                  <a @click="$refs.userRoleForm.userRole(record)">授权角色</a>
                </a-menu-item>
                <a-menu-item v-if="hasPerm('POST /sysUser/delete')">
  ...
  export default {
    ...
    data () {
      return {
        ...
        selectedRows: [],
        defaultExpandedKeys: [],
        sexDictTypeDropDown: [ { code: '1', value: '男' }, { code: '2', value: '女' } ],
    ...
    created () {
      ...
      if (this.hasPerm('POST /sysUser/edit') || this.hasPerm('POST /sysUser/resetPwd') || this.hasPerm('POST /sysUser/grantRole') || this.hasPerm('POST /sysUser/delete')) {
        this.columns.push({
          title: '操作',
          width: '150px',
          dataIndex: 'action',
